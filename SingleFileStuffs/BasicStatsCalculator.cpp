/********************************
This code gives us the set summation, mean, (variance, standard deviation => population or sample). 
...
float getSum(float showSet[], int size);

int main() {

multipleParams p;//declaring the structure to this scope
...
do {
    cout << "Enter array size: ";
      cin >> x; //Giving size to the array
        ...
        p.summationPart =  getSum(mySet, x);  
	  ...
	  cout << "The standard deviation is: " << p.standardDevi << endl;
          cout << "Set summation is: " << p.summationPart << endl;
...
float getSum(float showSet[], int size) {
         float setSummatio = 0.0;

	 for (int u = 0; u < size; u++) {
          setSummatio += showSet[u];
	 }
	 return setSummatio;
}
